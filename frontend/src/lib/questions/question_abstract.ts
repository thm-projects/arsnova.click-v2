...
import {QuestionI, ValidationStackTraceI} from "./QuestionI";
...
export abstract class AbstractQuestion {
  ...
  get answerOptionList(): Array<AnswerOptionI> {
    return this._answerOptionList;
  }

  set answerOptionList(value: Array<AnswerOptionI>) {
  ...
  private _answerOptionList: Array<AnswerOptionI>;

	constructor ({questionText = DefaultSettings.defaultSettings.question.text, timer = DefaultSettings.defaultSettings.question.timer, displayAnswerText = DefaultSettings.defaultSettings.answers.displayAnswerTextOnButtons, answerOptionList = Array<AnswerOptionI>()}) {
	 ...
	 * @param {AnswerOptionI} answerOption The AnswerOption instance to be added
	 ...
	 * @throws {Error} If the answerOption is not of type AnswerOptionI
	 */
	addAnswerOption (answerOption: AnswerOptionI, index: number = -1): void {
	 ...
	 */
	serialize (): Object {
		let answerOptionListSerialized = Array<AnswerOptionI>();
	...
	getValidationStackTrace (): Array<ValidationStackTraceI> {
		let result = Array<ValidationStackTraceI>();
		const questionTextWithoutMarkdownChars = this.getQuestionTextWithoutMarkdownChars().length;
